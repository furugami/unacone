...
async function fetchYoutubeByKeyword(
	...
	try {
		...
		// 概要欄にキーワードが含まれるだけの無関係な配信が混ざる。タイトル自体に「ポケモンユナイト」
		// または「ユナイト」を含むものだけに絞り込む（2026/09/03追加）。
		const TITLE_MATCH_KEYWORDS = [keyword, 'ユナイト'];
		const items = (videosData.items ?? []).filter(
			(item: { snippet: { title: string }; liveStreamingDetails?: Record<string, string | undefined> }) =>
				...
				TITLE_MATCH_KEYWORDS.some((k) => item.snippet.title.includes(k))
		);
